...
function getUserData(user) {
    ...
    for (var i = 0 ; i < user_data[user].verified.length ; i++) {
        ...
        clears++;
        progresses = progresses + '<li>' + list[rank].name + ' Верифнут </strong>(#'+(rank+1)+' / UP: '+roundNumber(getUserPoint(rank+1, 100, list[rank].percentToQualify, "144hz")*1, 3)+')<strong></a></li>'
    }
    ...
    Swal.fire({
        ...
        html : '<center><strong>Score : '+user_data[user].point + '<br>'+
            'Лучший прогресс : '+user_data[user].highest+'<br>'+
            'Пройдено : '+clears+' уровень(ней)<br>'+
            '<br>Список пройденного: '+progresses+'<br>'+
            '</strong></center>'
    });
